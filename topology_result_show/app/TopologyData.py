...
class TopologyData:
    ...
    def __init__(self,toolsBuild,mode,server_build,type="install",restDataUrl=""):
        self.dataList = []

        self.build = toolsBuild
        self.topoType = type
        self.resturl = restDataUrl
        self.data = None
        #self.legacyBuild = ""
        self.hostBuild = server_build
        self.topo_mode = mode
        self.getTopoData()
    
    def getTopoData(self):
        payload = {"buildNum": self.build,"topologyType":self.topoType}
        if self.topo_mode == "single Host":
            payload["topologyMode"]="single"
        elif self.topo_mode == "triple Host":
            payload["topologyMode"]="triple"

        payload["hostBuild"]=self.hostBuild
        payld = json.dumps(payload)
        result = requests.post(self.resturl,headers=TopologyData.head,data=payld)
        print("data from rest server ", result.text)
        dataList = json.loads(result.text)

        self.tp = TopologyPerBuildData(self.build,dataList)
        self.tp.build = self.build
        self.tp.parseBuildData()
        self.tp.mergerVMData()
    '''    
